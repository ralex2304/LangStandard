# Стандарт представления данных для компилятора

Данный стандарт использован в [компиляторе Lang](https://github.com/ralex2304/Lang).

## Содержание

1. [Стандарт AST - abstract syntax tree](#стандарт-ast---abstract-syntax-tree)
    1. [Общее](#общее)
    2. [Команды](#команды)
    3. [Типы узлов](#типы-узлов)
    4. [Формат](#формат)
        1. [Формат `DebugInfo`](#формат-debuginfo)
    5. [Файл](#файл)
2. [Стандарт IR - intermediate representation](#стандарт-ir---intermediate-representation)
    1. [Формат блока `IRNode`](#формат-блока-irnode)
    2. [Типы блоков `IRNodeType`](#типы-блоков-irnodetype)
        1. [Формат `IRVal`](#формат-irval)
        2. [Типы `CmpType`](#типы-cmptype)
        3. [Типы `JmpType`](#типы-jmptype)
        4. [Типы `MathOper`](#типы-mathoper)

## Стандарт AST - abstract syntax tree

### Общее

Кодировка файлов `cp1251`. Требуется для поддержки русского языка.
